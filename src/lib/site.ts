export const RDV_URL = "https://madamoon.fr/prise-de-rendez-vous/";
export const CATALOGUE_URL = "https://madamoon.fr/catalogue-des-robes/";

// Endpoint IA de la conseillère (Supabase Edge Function — la clé Anthropic reste côté serveur).
// La clé "anon" Supabase est publique par conception.
export const AI_ENDPOINT =
  "https://pfbyviktmtjnexvodehg.supabase.co/functions/v1/conseillere-madamoon";
export const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBmYnl2aWt0bXRqbmV4dm9kZWhnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2NzY4OTYsImV4cCI6MjA5ODI1Mjg5Nn0.IZ0bCe7DuOmCXt1e2CQQQUelsjE4NI3njRV9eN-OKtw";

// Préfixe des assets chargés hors next/image (textures 3D, HDRI)
export const ASSET_PREFIX = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
